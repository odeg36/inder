...
namespace LogicBundle\DataFixtures\ORM\Test;

use Doctrine\Common\DataFixtures\AbstractFixture;
use Doctrine\Common\DataFixtures\OrderedFixtureInterface;
use Doctrine\Common\Persistence\ObjectManager;
use LogicBundle\Entity\NivelPlan;
...
class FixturesNivelesPlanMetodologico extends AbstractFixture implements OrderedFixtureInterface, ContainerAwareInterface {
    ...
    public function load(ObjectManager $manager) {
        ...
        foreach ($tipos as $tipo) {
            $registro = $em->getRepository('LogicBundle:NivelPlan')->findOneBy(array('nombre' => $tipo['nombre']));
            if (!$registro) {
                $object = new NivelPlan();
